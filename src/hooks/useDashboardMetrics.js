...
function generateWeeklyAccess(openTrailCount) {
  const base = openTrailCount * 40
  const pattern = [0.55, 0.65, 0.58, 0.80, 0.75, 1.0, 0.85]
...
export function useDashboardMetrics() {
  return useMemo(() => {
    ...
    const alertTrails   = trails.filter(t => t.status === 'alert').length
    ...
    const accessDelta   = todayAccess - yesterdayAccess
    const accessDeltaLabel = accessDelta >= 0
      ? `+${accessDelta} vs ontem`
      : `${accessDelta} vs ontem`
