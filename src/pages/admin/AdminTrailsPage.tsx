...
export default function AdminTrailsPage() {
  const { trailsData, setTrailsData } = useAdminData();
  ...
  }, [editingTrail, trailStatusDraft, trailCondDraft, setTrailsData]);

  const executeDelete = useCallback(() => {
    if (!deleteTargetId) return;
    setTrailsData((prev) => prev.filter((t) => t.id !== deleteTargetId));
    setDeleteTargetId(null);
  }, [deleteTargetId, setTrailsData]);
